...
bool is_plain(const Blob::Ptr &blob) {
    ...
    for (size_t stride = 1, i = dims.size() - 1; i >= 0; --i) {
...
void BlobDumper::dumpAsTxt(std::ostream &stream) {
    ...
    const auto precision = _blob->getTensorDesc().getPrecision();
    switch (precision) {
        case Precision::FP32: {
            auto *blob_ptr = _blob->buffer().as<float*>();
            for (size_t i = 0; i < data_size; i++) {
                const auto value = blob_ptr[i];
                stream << std::setprecision(9) << (abs(value) <= 1e-10 ? 0.f : value) << std::endl;
            }
            break;
        }
        case Precision::BF16:
        {
            ...
            for (size_t i = 0; i < data_size; i++) {
                ...
                stream << std::setprecision(9) << fn << std::endl;
            }
            break;
        }
        case Precision::I32: {
            ...
            for (size_t i = 0; i < data_size; i++)
                stream << blob_ptr[i] << std::endl;
            break;
        }
        case Precision::I16: {
            auto *blob_ptr = _blob->buffer().as<int16_t*>();
            for (size_t i = 0; i < data_size; i++)
                stream << static_cast<int>(blob_ptr[i]) << std::endl;
            break;
        }
        case Precision::I64: {
            auto *blob_ptr = _blob->buffer().as<int64_t*>();
            for (size_t i = 0; i < data_size; i++)
                stream << static_cast<int64_t>(blob_ptr[i]) << std::endl;
            break;
        }
        case Precision::U16: {
...
void BlobDumper::dump(const std::string &dump_path) {
    ...
    if (!dump_file.is_open())
        THROW_IE_EXCEPTION << "Dumper cannot create dump file: " << dump_path;
