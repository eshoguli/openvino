...
void ArmPlugin::pass::ConvBiasFusionBase::registerMatcher(const std::string& name) {
    ...
    register_matcher(std::make_shared<ngraph::pattern::Matcher>(eltwise_pattern, name), [=](ngraph::pattern::Matcher& m) {
        ...
        auto quantized = (conv->get_input_element_type(Inputs::Data) == ngraph::element::u8 ||
                          conv->get_input_element_type(Inputs::Data) == ngraph::element::i8 ||
                          conv->get_input_element_type(Inputs::Weights) == ngraph::element::u8 ||
                          conv->get_input_element_type(Inputs::Weights) == ngraph::element::i8);
        ...
        if (quantized) {
            new_conv = std::make_shared<ngraph::op::TypeRelaxed<Conv>>(
                std::vector<ngraph::element::Type>{ngraph::element::f32, ngraph::element::f32, ngraph::element::f32},
                std::vector<ngraph::element::Type>{conv->get_output_element_type(0)},
                ngraph::op::TemporaryReplaceOutputType(conv->input_value(Inputs::Data), ngraph::element::f32).get(),
                ngraph::op::TemporaryReplaceOutputType(conv->input_value(Inputs::Weights), ngraph::element::f32).get(),
                ngraph::op::TemporaryReplaceOutputType(new_bias, ngraph::element::f32).get(),
                conv->get_strides(),
                conv->get_pads_begin(),
                conv->get_pads_end(),
                conv->get_dilations(),
