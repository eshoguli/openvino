...
bool ArmPlugin::pass::ArmOptimizations::run_on_function(std::shared_ptr<ngraph::Function> f) {
    ...
    ngraph::pass::VisualizeTree{"init_graph.dot"}.run_on_function(f);
    ...
    if (quantized) {
        ...
        // manager.register_pass<StoreWeightsQuantizationInfo>();
    }
    ...
    ngraph::pass::VisualizeTree{"Before_lpt.dot"}.run_on_function(f);
    ...
    if (quantized) {
        ...
        auto params = LayerTransformation::Params(
            ...
            true)
            .setPrecisionsOnActivations({ngraph::element::u8});
    ...
    ngraph::pass::VisualizeTree{"Before_merge.dot"}.run_on_function(f);
    ...
    if (quantized) {
        ngraph::pass::Manager manager;
        manager.register_pass<pass::FixQuantizedSubtractOutputDataType>();
        manager.register_pass<pass::FixMulInputConvert>();
        manager.register_pass<pass::DetectMaybeQuantized>();
        manager.register_pass<pass::QuantizeFusion>();
        manager.register_pass<ngraph::pass::ConstantFolding>();
        manager.register_pass<pass::ConvertQuantizeDequantize>();
