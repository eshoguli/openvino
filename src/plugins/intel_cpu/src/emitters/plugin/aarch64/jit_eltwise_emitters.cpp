...
void jit_add_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    //std::cout << "jit_add_emitter::emit_isa" << std::endl;
...
//    std::cout << "jit_clamp_emitter::emit_isa" << std::endl;
    if (host_isa_ == dnnl::impl::cpu::aarch64::asimd) {
        emit_isa<dnnl::impl::cpu::aarch64::asimd>(in_vec_idxs, out_vec_idxs);
    } else {
        OV_CPU_JIT_EMITTER_THROW("Can't create jit eltwise kernel");
...
void jit_sigmoid_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    std::vector<size_t> exp_aux_vec_idxs;
    exp_aux_vec_idxs.assign(aux_vec_idxs.begin(), aux_vec_idxs.end());
    std::vector<size_t> exp_aux_gpr_idxs = { aux_gpr_idxs[0] };
...
jit_tanh_emitter::jit_tanh_emitter(dnnl::impl::cpu::aarch64::jit_generator *host,
    ...
    //sigmoid_emitter = std::make_unique<jit_exp_emitter>(h, host_isa, node);
}
...
jit_tanh_emitter::jit_tanh_emitter(dnnl::impl::cpu::aarch64::jit_generator *host,
    ...
    //sigmoid_emitter = std::make_unique<jit_exp_emitter>(h, host_isa, exec_prc);
}
...
//    std::cout << "jit_tanh_emitter::emit_isa" << std::endl;
    if (exec_prc_ != ov::element::f32) {
        OPENVINO_THROW("unsupported precision: " + exec_prc_.to_string());
    }
