...
#define OV_CPU_JIT_EMITTER_LOG(exec_prc_) std::cout << __PRETTY_FUNCTION__ << ", exec_prc_: " << exec_prc_ << std::endl;
//#define OV_CPU_JIT_EMITTER_LOG(exec_prc_)
...
//    const auto v16 = dnnl::impl::utils::bit_cast<int16_t, float16>(value);
//    const auto v16_2 = dnnl::impl::utils::bit_cast<float16, int16_t>(v16);
//    const auto diff16 = abs(value - static_cast<float>(v16_2));
//
//    const auto v32 = dnnl::impl::utils::bit_cast<int, float>(value);
//    const auto v32_2 = dnnl::impl::utils::bit_cast<float, int>(v32);
//    const auto diff32 = abs(value - v32_2);
...
void jit_abs_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    OV_CPU_JIT_EMITTER_LOG(exec_prc_);
...
void jit_power_static_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    if (std::floor(power) == power && power > 0) {
        ...
        if (power != -1) {
            std::cout << "jit_power_static_emitter::emit_isa: power=" << power << std::endl;
        }
