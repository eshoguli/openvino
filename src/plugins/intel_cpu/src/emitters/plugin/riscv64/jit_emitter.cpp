...
void jit_emitter::load(const VReg& rd, const float imm, const Reg& aux) const {
    ...
    h->li(aux, this->float2int(imm));
    FReg aux_f(0);
    h->fmv_w_x(aux_f, aux);

    h->vxor_vv(rd, rd, rd);
    h->vfadd_vf(rd, rd, aux_f);
    ////h->vfmv_s_f(target, aux_f);
}
...
int32_t jit_emitter::float2int(const float x) {
    ...
    const auto version = ov::util::getenv_int("OV_FLOAT2INT", 0);
    if (version == 0) {
        y_ptr[0] = x_ptr[0];
        y_ptr[1] = x_ptr[1];
        y_ptr[2] = x_ptr[2];
