...
void jit_power_static_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    std::cout << "debug: jit_power_static_emitter::emit_isa: " << exec_prc_ << std::endl;
    std::cout << "\tin_vec_idxs: ";
    for (const auto indx : in_vec_idxs) { std::cout << indx << ", "; }
    std::cout << std::endl << "\taux_vec_idxs: ";
    for (const auto indx : aux_vec_idxs) { std::cout << indx << ", "; }
    std::cout << std::endl << "\tout_vec_idxs: ";
    for (const auto indx : out_vec_idxs) { std::cout << indx << ", "; }
    std::cout << std::endl << "\taux_gpr_idxs: ";
    for (const auto indx : aux_gpr_idxs) { std::cout << indx << ", "; }
    std::cout << std::endl;
    ...
    if (scale != 1.f) {
        //load_table_val("scale", Reg(aux_gpr_idxs[0]), aux);
        load(aux, scale, Reg(aux_gpr_idxs[0]));

        const auto op = ov::util::getenv_int("OV_OP", 0);
        if (op == 0) {
            h->vfmul_vv(dst, src(), aux);
        } else if (op == 1) {
            h->vfadd_vv(dst, src(), aux);
        } else {
            h->vxor_vv(dst, dst, dst);
            h->vfadd_vv(dst, dst, aux);
        }
        get_from_dst = true;
    }

    if (shift != 0.f) {
        //load_table_val("shift", Reg(aux_gpr_idxs[0]), aux);
