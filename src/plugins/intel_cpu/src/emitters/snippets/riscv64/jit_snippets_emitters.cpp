...
jit_nop_emitter::jit_nop_emitter(jit_generator* h, cpu_isa_t isa, const ExpressionPtr& expr) : riscv64::jit_emitter(h, isa) {
...
void jit_broadcast_move_emitter::emit_impl(const std::vector<size_t>& in,
    ...
    if (host_isa_ == ov::intel_cpu::riscv64::asimd) {
        emit_isa<ov::intel_cpu::riscv64::asimd>(in, out);
    } else {
...
void jit_broadcast_move_emitter::emit_isa(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    using TReg = typename ov::intel_cpu::riscv64::cpu_isa_traits<isa>::TReg;
    TReg src = TReg(in[0]);
    TReg dst = TReg(out[0]);
