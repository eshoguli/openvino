...
public:
    explicit CompiledSnippetCPU(std::unique_ptr<ov::intel_cpu::riscv64::jit_generator> h);
    const uint8_t* get_code() const override;
    size_t get_code_size() const override;
    bool empty() const override;

private:
    const std::unique_ptr<const ov::intel_cpu::riscv64::jit_generator> h_compiled;
};

class CPUTargetMachine : public snippets::TargetMachine {
public:
    explicit CPUTargetMachine(ov::intel_cpu::riscv64::cpu_isa_t host_isa);
    ...
    ov::intel_cpu::riscv64::cpu_isa_t get_isa() const;

private:
    std::unique_ptr<ov::intel_cpu::riscv64::jit_generator> h;
    ov::intel_cpu::riscv64::cpu_isa_t isa;
};

class CPUGenerator : public snippets::Generator {
public:
    CPUGenerator(ov::intel_cpu::riscv64::cpu_isa_t isa);
