...
using jit_generator = ov::intel_cpu::riscv64::jit_generator;
using cpu_isa_t = ov::intel_cpu::riscv64::cpu_isa_t;
using ExpressionPtr = ov::snippets::lowered::ExpressionPtr;

inline static std::vector<Reg> transform_idxs_to_regs(const std::vector<size_t>& idxs) {
    std::vector<Reg> regs(idxs.size(), Reg(0));
    std::transform(idxs.begin(), idxs.end(), regs.begin(), [](size_t idx){return Reg(idx);});
    return regs;
}
...
jit_kernel_emitter::jit_kernel_emitter(jit_generator* h, cpu_isa_t isa, const ov::snippets::lowered::ExpressionPtr& expr)
    : jit_emitter(h, isa), reg_runtime_params_idx(Xbyak_riscv::x0.getIdx()) {
...
jit_kernel_static_emitter::jit_kernel_static_emitter(ov::intel_cpu::riscv64::jit_generator* h,
                                                     ov::intel_cpu::riscv64::cpu_isa_t isa,
                                                     const ov::snippets::lowered::ExpressionPtr& expr)
    : jit_kernel_emitter(h, isa, expr), reg_indexes_idx(Xbyak_riscv::x0.getIdx()) {
...
void jit_kernel_static_emitter::init_data_pointers(const std::vector<Reg>& data_ptr_regs) const {
...
jit_kernel_dynamic_emitter::jit_kernel_dynamic_emitter(ov::intel_cpu::riscv64::jit_generator* h,
                                                       ov::intel_cpu::riscv64::cpu_isa_t isa,
                                                       const ov::snippets::lowered::ExpressionPtr& expr)
...
void jit_kernel_dynamic_emitter::init_data_pointers(const std::vector<Reg>& data_ptr_regs) const {
//    XReg reg_runtime_params = XReg(static_cast<int>(reg_runtime_params_idx));
//
//    const auto num_params = num_inputs + num_outputs;
//    for (size_t i = 0; i < num_unique_buffers; ++i) {
