...
public:
    jit_emitter(dnnl::impl::cpu::aarch64::jit_generator* host,
                ...
                Emitter(nullptr), h(host), host_isa_(host_isa), exec_prc_(exec_prc)/*, l_table(new Label())*/, in_out_type_(in_out_type) {
    ...
    jit_emitter(dnnl::impl::cpu::aarch64::jit_generator* host,
                ...
                InferenceEngine::Precision exec_prc = InferenceEngine::Precision::FP32,
                emitter_in_out_map in_out_type = emitter_in_out_map::vec_to_vec) :
                Emitter(n), h(host), host_isa_(host_isa), exec_prc_(exec_prc)/*, l_table (new Label())*/, in_out_type_(in_out_type) {
        //k_mask = Xbyak::Opmask(1); // FIXME: in general case we need preserve k_mask state as well
    }
