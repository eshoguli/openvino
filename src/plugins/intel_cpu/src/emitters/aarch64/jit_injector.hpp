...
struct jit_injector_f32 {
    using TReg = typename dnnl::impl::cpu::aarch64::cpu_isa_traits<isa>::TReg;
    using TRegS = typename dnnl::impl::cpu::aarch64::cpu_isa_traits<isa>::TRegS;

    jit_injector_f32(dnnl::impl::cpu::aarch64::jit_generator* host,
                     const Xbyak_aarch64::XReg x_table) : x_table(x_table), l_table(new Xbyak_aarch64::Label()), h(host) {
    }
    ...
    size_t table_off(key_t key, size_t key_off_val_shift = 0) const {
    ...
    TReg table_val(key_t key, TReg zreg, size_t key_off_val_shift = 0) const {
    ...
    void push_arg_entry_of(const key_t key, const table_entry_val_t val, const bool broadcast) {
