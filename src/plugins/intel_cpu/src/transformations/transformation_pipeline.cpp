...
void Transformations::UpToCpuSpecificOpSet() {
    ...
    ngraph::pass::VisualizeTree("svg/cpu.original.svg.dot").run_on_model(model);
    ...
    for (const auto op : model->get_ordered_ops()) {
        const auto if_op = ov::as_type_ptr<ov::opset8::If>(op);
        if (if_op != nullptr) {
            ngraph::pass::Serialize("svg/cpu.if_then.1.xml", "svg/cpu.if_then.1.bin").run_on_model(if_op->get_then_body());
            ngraph::pass::VisualizeTree("svg/cpu.if_then.1.svg").run_on_model(if_op->get_then_body());

            ngraph::pass::Serialize("svg/cpu.if_else.1.xml", "svg/cpu.if_else.1.bin").run_on_model(if_op->get_else_body());
            ngraph::pass::VisualizeTree("svg/cpu.if_else.1.svg").run_on_model(if_op->get_else_body());
        }
    }

    ngraph::pass::Serialize("svg/cpu.common.xml", "svg/cpu.common.bin").run_on_model(model);
    ngraph::pass::VisualizeTree("svg/cpu.common.svg.dot").run_on_model(model);

    if (useLpt) {
        Lpt(hasINT16orINT32Levels, defaultPrecisions);
        ngraph::pass::Serialize("svg/cpu.lpt.xml", "svg/cpu.lpt.bin").run_on_model(model);
        ngraph::pass::VisualizeTree("svg/cpu.lpt.svg.dot").run_on_model(model);
    }
    ...
    for (const auto op : model->get_ordered_ops()) {
        const auto if_op = ov::as_type_ptr<ov::opset8::If>(op);
        if (if_op != nullptr) {
            ngraph::pass::Serialize("svg/cpu.if_then.2.xml", "svg/cpu.if_then.2.bin").run_on_model(if_op->get_then_body());
            ngraph::pass::VisualizeTree("svg/cpu.if_then.2.svg").run_on_model(if_op->get_then_body());

            ngraph::pass::Serialize("svg/cpu.if_else.2.xml", "svg/cpu.if_else.2.bin").run_on_model(if_op->get_else_body());
            ngraph::pass::VisualizeTree("svg/cpu.if_else.2.svg").run_on_model(if_op->get_else_body());
        }
    }

    ngraph::pass::Serialize("svg/cpu.transformed.xml", "svg/cpu.transformed.bin").run_on_model(model);
    ngraph::pass::VisualizeTree("svg/cpu.transformed.svg.dot").run_on_model(model);
}
