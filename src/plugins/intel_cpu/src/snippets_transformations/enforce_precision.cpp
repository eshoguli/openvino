...
namespace {
bool all_input_precisions_are_not_equal(const std::shared_ptr<Node>& node, const element::Type& target) noexcept {
    const auto& inputs = node->inputs();
    return std::all_of(
        inputs.begin(),
        inputs.end(),
        [&target](const Input<Node>& input) { return input.get_element_type() != target; });
}
} // namespace
...
bool EnforcePrecision::run_on_model(const std::shared_ptr<ov::Model>& f) {
    ...
    for (const auto& op : f->get_ordered_ops()) {
        ...
        if (op_desc.require_bf16_isa && !bf16_isa) {
            // just to check with assert: precision enforcement logic doesn't define precision for not supported operations
            assert(all_input_precisions_are_not_equal(op, target));
            continue;
        }
