...
bool checkAndInitPostOps(const PostOps &postOps, arm_compute::GEMMInfo& info) {
    // Add postops
    if (!postOps.empty() && postOps.size() == 1) {
        if (const auto activation = std::dynamic_pointer_cast<ActivationPostOp>(postOps[0])) {
            auto activation_info = info.activation_info();
            auto result = getActivationLayerInfo(convertToEltwiseAlgorithm(
                    activation->type()),
                    activation_info,
                    activation->alpha(),
                    activation->beta(),
                    activation->gamma());
            if (result) {
                info.set_activation_info(activation_info);
            }
            return result;
        }
    }
...
void initFCAttrs(const FCAttrs &attrs,
    ...
    checkAndInitPostOps(postOps, gemmInfo);
