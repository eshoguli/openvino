...
ACLMatMulExecutor::ACLMatMulExecutor(const GEMMAttrs &attrs, const PostOps &postOps,
                                     const MemoryArgs &memory,
                                     const ExecutorContext::CPtr context) {
    aclTensorAttrs.hasLayoutTypeNHWC = memory.at(ARG_SRC)->getDescPtr()->hasLayoutType(LayoutType::nspc);
}

bool ACLMatMulExecutor::supports(const GEMMConfig &config) {
    // MatMul:
...
void ACLMatMulExecutor::updateTensorsShapes(ACLMemoryShapes& aclMemoryShapes) {
...
arm_compute::Status ACLMatMulExecutor::validateTensorsInfo(const ACLMemoryInfo & aclMemoryInfos) {
...
ACLFunction ACLMatMulExecutor::configureFunction(const ACLMemoryTensors & aclMemoryTensors) {
...
ACLInfo ACLMatMulExecutor::initTensorInfo(const arm_compute::TensorShape& tensorShape,
