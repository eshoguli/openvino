...
void Snippet::generate(const jit_snippets_compile_args* jcp) {
    ...
    if (context->getConfig().enforceBF16) {
        ...
        const bool bf16_isa = dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx512_core_fp16);
        // TODO: to test only
        std::cout << "Snippet::generate: " << (bf16_isa ? "BF16 is supported" : "BF16 is not supported") << std::endl;
        pre_dialect.register_pass<pass::EnforcePrecision>(element::f32, element::bf16, bf16_isa);
    }
