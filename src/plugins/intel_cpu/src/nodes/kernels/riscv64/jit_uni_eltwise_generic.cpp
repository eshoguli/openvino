...
//    }
    ...
    for (size_t i = 1; i < eltwise_data_.size(); ++i) {
        ...
        input_count += emitter->get_inputs_count();
...
//             }
            ...
            for (size_t i = 0; i < jep.inputs_number; i++) {
                if (jep.src_size[i] != 1) {
                    add(get_aux_gpr(i), x0, get_src_reg(i));
                }
            }
            ...
            L(unroll_loop_label2);
            {
                ...
                for (size_t i = 0; i < jep.inputs_number; i++) {
                    if (jep.src_size[i] != 1) {
                        ...
                        vle32_v(get_vmm_reg(i, lmul), get_aux_gpr(i));
                        add(get_aux_gpr(i), get_aux_gpr(i), t0);
                    }
                }

                if (jit_compute) {
                    compute_eltwise_op(lmul, input_count);
                } else {
                ...
                if (jit_loop) {
                    ...
                    std::cout << "loop is ignored" << std::endl;
                }
            }

            for (size_t i = 0; i < jep.inputs_number; i++) {
                if (jep.src_size[i] == jep.dst_size) {
                    li(get_aux_gpr(i), jep.src_prc[i].size() * min_src_size);
                    add(get_src_reg(i), get_src_reg(i), get_aux_gpr(i));
                }
            }
...
void jit_uni_eltwise_generic::compute_eltwise_op(const LMUL lmul, const uint32_t input_reg_count) {
...
void jit_uni_eltwise_generic::apply_post_ops(const LMUL lmul, const uint32_t input_reg_count) {
