...
void CPUTestsBase::CheckPluginRelatedResultsImpl(const std::shared_ptr<const ov::Model>& function, const std::set<std::string>& nodeType) const {
    ...
    //     auto getExecValue = [](const std::map<std::string, ov::Any>& rtInfo, const std::string & paramName) -> std::string {
    //         auto it = rtInfo.find(paramName);
    //         IE_ASSERT(rtInfo.end() != it);
    //         return it->second.as<std::string>();
    //     };
    //     const auto & rtInfo = node->get_rt_info();
    //     const auto layer_type = getExecValue(rtInfo, ExecGraphInfoSerialization::LAYER_TYPE);
