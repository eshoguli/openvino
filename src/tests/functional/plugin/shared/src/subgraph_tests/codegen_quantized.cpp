...
void CodegenQuantized::SetUp() {
    ...
    //// if we create TypeRelaxed types before common transformations then constant folding will fail
    //const auto original_subtract = std::make_shared<ngraph::opset1::Subtract>(
    //    input,
    //    // TODO: f32 & u8
    //    //std::make_shared<ngraph::opset1::Constant>(ov::element::f32, ov::Shape{}, std::vector<float>{10.f}));
    //    std::make_shared<ngraph::opset1::Constant>(inputPrecision, ov::Shape{}, std::vector<float>{10.f}));
    //const auto subtract = std::make_shared<ngraph::op::TypeRelaxed<ngraph::opset1::Subtract>>(*original_subtract, outputPrecision);
