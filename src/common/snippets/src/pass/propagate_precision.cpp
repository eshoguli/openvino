    ...
    for (const auto& op : f->get_ordered_ops()) {
        ...
        if (!supported_precisions.empty() &&
            ...
            for (auto i = 0ull; i < op->get_input_size(); ++i) {
                ...
                if (parent_output.get_element_type() != precision) {
                    ...
                    if (existing_convert == nullptr) {
                        ...
                        if (precision == parent_output.get_element_type()) {
            ...
            if (auto type_relaxed_node = std::dynamic_pointer_cast<ov::op::TypeRelaxedBase>(op)) {
                // TODO: to keep previous functionality
