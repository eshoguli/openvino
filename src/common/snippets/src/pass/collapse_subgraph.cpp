...
//        }
//    }
//    const auto v1 = std::all_of(inputs.begin(), inputs.end(), [&](const Input<const Node>& in) {return  supported(in.get_tensor());});
//    const auto v2 = std::all_of(outputs.begin(), outputs.end(), [&](const Output<const Node>& out) {return  supported(out.get_tensor());});
//
//    if (is_type<opset1::FakeQuantize>(n)) {
...
TokenizeSnippets::TokenizeSnippets() {
    ...
    auto label = std::make_shared<pattern::op::Label>(pattern::any_input(),
        [](const std::shared_ptr<const Node> &n) {
            const auto v1 = GetSnippetsNodeType(n);
            //const auto v2 = AppropriateForSubgraph(n);
            //if (is_type<opset1::FakeQuantize>(n)) {
            //    std::cout << "TokenizeSnippets" << std::endl;
            //}
